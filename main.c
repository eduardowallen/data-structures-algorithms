...
int main() {
	List* List1, *List2;			// Creates lists L1 and L2
	Node* tempnode;					// Creates a temporary node that we can use within the main function
	...
	tempnode = createNode(3);		// Creates a node (overwrites the current data in tempnode) and stores it in tempnode
	insert(&List1, tempnode);		// Inserts the created node into list 1
	insert(&List2, tempnode);		// Inserts the created node into list 2
	print(&List1);					// Prints list 1
	print(&List2);					// Prints list 2
